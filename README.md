# Geolocation Data Aggregation (MySQL Focus)
...
*   `schema.sql`: Database creation and table setup.
*   `seeds.sql`: Simulated data insertion.
...
| :--- | :--- | :--- |
| `checkin_id` | `INT` | Primary key for the event. |
| `user_id` | `VARCHAR` | Unique identifier for each user. |
| `city` | `VARCHAR` | The city where the check-in occurred. |
| `timestamp` | `DATETIME` | The exact time and date of the check-in. |
